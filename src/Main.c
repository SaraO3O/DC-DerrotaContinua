...
void inicializaNpcs(float* dirx, float* diry) {
	npc[0].id = 0;
	strcpy_s(npc[0].nome, 20, "veterana");
	npc[0].x = 1120 + (*dirx + 500); npc[0].y = 1080 + (*diry + 350);
	npc[0].w = 52; npc[0].h = 52;
	strcpy_s(npc[0].mensagem, 500, "quer ter uma batalha introdutoria?\n");
}
...
void swap(Rank* a, Rank* b) {
	Rank temp;
	strcpy_s(temp.Nome, 30, a[0].Nome);
	temp.RA = a[0].RA;
	strcpy_s(a[0].Nome, 30, b[0].Nome);
	a[0].RA = b[0].RA;
	strcpy_s(b[0].Nome, 30, temp.Nome);
	b[0].RA = temp.RA;
}
...
Rank* criarranking(Rank* a, int* n) {
	...
	strcpy_s(teste, 30, player.nome);
	ira = player.IRA;

	if (pe < 10) {
		a[pe].RA = ira;
		strcpy_s(a[pe].Nome, 30, teste);
		pe = pe + 1;
		bubbleSort(a, pe - 1);
		*n = pe;
	}
		...
		a[0].RA = ira;
		strcpy_s(a[0].Nome, 30, teste);
		bubbleSort(a, pe - 1);
	}

	fopen_s(&p, "Rank.bin", "wb");
...
void ranking(int* estado) {
	...
	fopen_s(&qtd, "Rank.bin", "rb");


	if (qtd == NULL) {
		_fcloseall();
		fopen_s(&qtd, "Rank.bin", "wb");
		_fcloseall();
	}
	else {
